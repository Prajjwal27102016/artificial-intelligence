...
def interactive_edge_detection(image_path):
   ...
   while True:
       ...
       if choice == "1":
           # Sobel Edge Detection — combine X and Y gradients via magnitude, not bitwise OR
           sobel_x = cv2.Sobel(gray_image, cv2.CV_64F, 1, 0, ksize=3)
           sobel_y = cv2.Sobel(gray_image, cv2.CV_64F, 0, 1, ksize=3)
           magnitude = cv2.magnitude(sobel_x, sobel_y)          # sqrt(x^2 + y^2)
           combined_sobel = cv2.convertScaleAbs(magnitude)      # properly scale/clip to uint8
           ...
           # Gaussian Smoothing — kernel size must be odd & positive
           ...
           # Median Filtering — kernel size must be odd & positive
           kernel_size = get_odd_kernel_size("Enter kernel size (odd number)")
...
interactive_edge_detection('cat blend.jpg')
